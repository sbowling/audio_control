...
class Echo(EffectBase):
    def __init__(self, master, client, config_file="configs/echo.json"):
        ...
        # Title
        ctk.CTkLabel(self, text="Echo", font=("Impact", 24), text_color="#aaaaaa").grid(row=0, column=0, pady=15)
        ...
        # Time Knob
        self.create_knob(self.knob_frame, "TIME", self.config.get("time_register", 110))
        
        # Spacer
        ctk.CTkLabel(self.knob_frame, text="   ").pack(side="left")
        
        # Regen Knob
        self.create_knob(self.knob_frame, "REGEN", self.config.get("regen_register", 111))
        
        # Spacer
        ctk.CTkLabel(self.knob_frame, text="   ").pack(side="left")

        # Mix Knob
        self.create_knob(self.knob_frame, "MIX", self.config.get("mix_register", 112))
        ...
        def process_write(helper):
            if self.client and self.client.connected and self.power_state:
                try:
                    ...
                    print(f"Echo Knob Write Error: {e}")
        ...
        try:
            for item in self.knobs:
                ...
                try:
                    ...
                    print(f"Read Error Echo {item['reg']}: {e}")
